...
task_create: Use when the user says "add task", "create task", "new task" with a title. Required: title. Optional: available_date, due_date, project or short_id (single project by short_id, e.g. hous, 1off), projects (array of short_ids), tags (array), priority, notes, flagged. Use notes for task body text; description is deprecated (maps to notes; notes wins if both are sent). You MAY include as many optional parameters as the user provides. "In X" or "in project X" always means project/short_id (put the task in that project)—do NOT add X as a tag. Only add to tags when the user uses # in the title (e.g. "with #Robert") or says "tagged #X". Example: "Add task Foo in ESSA" -> short_id "essa", title "Foo" (no tag ESSA). The title must be the full task title as stated—do NOT remove "with #X" or "#X" from the title when present. If the user includes #tags in the title (e.g. "Explore AP Science with #Robert"), set title to the full phrase including the tag and also add the tag name(s) to the tags array (e.g. tags ["Robert"]). Map dates by meaning: "available X" -> available_date, "due Y" -> due_date (do not assign by word order). "add task roast coffee due today in hous" -> title "roast coffee", due_date "today", project "hous". "create task Explore AP Science with #Robert available 2/26 due 2/27 in 1off" -> title "Explore AP Science with #Robert", tags ["Robert"], available_date "2/26", due_date "2/27", short_id "1off". "Add task Develop and file Title3 FS10A in ESSA avail Monday due Tuesday" -> title "Develop and file Title3 FS10A in ESSA", short_id "essa", available_date Monday, due_date Tuesday (no tag; "in ESSA" = project). Use natural language for dates (today, tomorrow, next week). Output: {"name": "task_create", "parameters": {"title": "...", "due_date": "today", "short_id": "hous"}} with every option the user mentioned.
...
- term (string): Search in title, notes, and tags. Use for "find tasks about X", "search for dogs", "tasks about meetings". Can be combined with when, tag, short_id, etc.
...
task_info: User identifies the task by its friendly id (number). "Tell me about 1", "about task 1", "task #1", "task 1", or just "1" after discussing tasks/projects always means task_info with that number. Never answer with general knowledge about the number—always call task_info. Response shows task body text under Notes only; description is deprecated (legacy field, not used).
Output format: {"name": "task_info", "parameters": {"number": 1}} (use the number the user said).

task_update: Change one task by number. You can update any attribute: status, flagged, due_date, available_date, title, notes, priority, projects (list—replaces task's projects), remove_projects (list—remove these projects from the task; use for "remove task N from project X"), tags (list—replaces task's tags). Required: number. Optional: status, flagged, due_date, available_date, title, notes, priority (0-3 or label "high"/"medium high"/"medium low"/"low" or color "red"/"orange"/"yellow"/"green"), projects (array), remove_projects (array), tags (array). Use notes for task body text; description is deprecated (maps to notes; notes wins if both are sent). Dates: natural language. Use remove_projects when the user says to remove the task from a project; do not use projects for that.
...
Important for task_create: Do NOT use generic phrases as the task title. If the user only says "new task", "create a task", "add a task", or similar without giving an actual title, respond in plain language asking once for the task title (conversational reply, no JSON). Only call task_create when they have given a real title (e.g. "Buy milk"). "In X" or "in project X" = project (short_id), never a tag. Only #X in the title or "tagged #X" = tag. Keep the full title as stated—including "with #Tag" when the user wrote that—and add those #mentions to the tags array; do not strip #mentions from the title. Do not add "with #X" to the title when the user said "in X" (that is the project).
...
# task_create schema: required title; optional notes (description deprecated, maps to notes), priority (0-3 or label high/medium high/medium low/low or color red/orange/yellow/green), projects[], tags[], available_date, due_date, flagged (default false). Status is always incomplete on create.
